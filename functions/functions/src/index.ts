...
exports.createReport = functions.firestore
    ...
    .onCreate((snap, context) => {
        ...
        for(let i=0;i<snap.data().wildBoar.length ;i++){
            const tmp:number = snap.data().wildBoar[i]
            total = total + tmp
        }
        ...
        return admin.firestore().collection('users').doc(uid)
            ...
            .then(
                admin.firestore().doc(`/reports/${docID}`).set(report)
            .then(
                snap.ref.delete())
            .then(
                admin.firestore().collection("userTokens").doc(uid).get()
                .then((docUserTokens:DocumentSnapshot) => {
                    if( docUserTokens.exists) {
                        console.log('Uid: ' + docUserTokens.data()?.token);
                        admin.firestore().collection('users').doc(uid).get()
                        .then(
                        (docUser: DocumentSnapshot) => {
                            admin.messaging().sendToDevice(docUserTokens.data()?.token,
                                {
                                    notification: {
                                        title: `Dziękujemy za zgłoszenie.`,
                                        body:
                                            "Twoja dotychczasowa liczba zgłoszonych dzików to " + (docUser.data()?.numberOfWBR)}}
                            )
                        })
                    }
            }))
